...
import { eventType, eventBehavior } from '@/data/defaultsConfig';
    ...
    <el-form @submit="(e) => e.preventDefault()">
      ...
      <el-form-item :label="dep.name" v-for="dep in depList?.depend">
        ...
        <el-select v-model="dep.bindData" :placeholder="dep.option.placeholder" v-else-if="dep.type === 'select'">
          <el-option v-for="item in dep.option.list" :key="item.value" :label="item.name" :value="item.value" :disabled="item.disabled"> </el-option>
        </el-select>
      </el-form-item>
