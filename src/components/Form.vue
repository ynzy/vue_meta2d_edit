  ...
  <el-collapse>
    <el-collapse-item v-for="(item, index) in props.formList" :title="item.title" :key="index">
      <el-form @submit="(e) => e.preventDefault()">
        <el-form-item v-for="(i, childIndex) in item.children" :label="i.title" :key="childIndex">
          <!-- 输入框 -->
          ...
          <el-select v-model="i.bindProp[i.prop]" :placeholder="i.option.placeholder" v-else-if="i.type === 'select'" @[i.event]="i.func">
            <el-option v-for="item in i.option.list" :key="item.value" :label="item.label" :value="item.value" :disabled="item.disabled">
